...
<main class="mt-5 pt-3">
    ...
    <div class="container mb-5">
        <div class="row justify-content-center align-items-center">
            <div class="col-md-10">
                <form action="/admin/items" method="POST" class="row g-3" enctype="multipart/form-data" autocomplete="off">
                    @if (Session::get('fail'))
                    ...
                    <div class="col-md-6">
                        ...
                        <input name="itemId" type="number" class="form-control rounded-0 addIteminput" readonly value="{{ $next_id }}">
                    </div>
                    <div class="col-md-6">
                        <label for="skuNo" class="form-label productAddLable">SKU No</label>
                        <input name="skuNo" type="text" class="form-control rounded-0 addIteminput @error('skuNo') is-invalid @enderror " placeholder="Enter Item SKU No" value="{{ old('skuNo') }}">
                    ...
                    <div class="col-12">
                        ...
                        <input name="itemName" type="text" class="form-control rounded-0 addIteminput @error('itemName') is-invalid @enderror" placeholder="Enter Item Name" value="{{ old('itemName') }}">
                    ...
                    <div class="col-6">
                        ...
                        <select name="brand" id="brand" class="form-select  rounded-0 addIteminput @error('brand') is-invalid @enderror" >
                            ...
                            @foreach ($brands as $oneBrandItem)
                                <option value="{{ $oneBrandItem->brand }}"{{old('brand') == $oneBrandItem->brand  ? 'selected' : ''}} >{{ $oneBrandItem->brand }}</option>   
                            @endforeach
                        </select>
                    ...
                    <div class="col-3">
                        ...
                        <input name="price" type="text" class="form-control rounded-0 addIteminput @error('price') is-invalid @enderror " placeholder="Enter Item price" value="{{ old('price') }}">
                        <span class="text-danger">@error('price'){{ $message }}@enderror</span>
                    </div>
                    <div class="col-3">
                        <label for="quantity" class="form-label productAddLable">Quantity</label>
                        <input name="quantity" type="text" class="form-control rounded-0 addIteminput @error('quantity') is-invalid @enderror " placeholder="Enter Item Quantity" value="{{ old('quantity') }}">
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-6">
                                                    <div class="mb-3 form-check">
                                                        <input name="category[]" type="checkbox" class="form-check-input @error('category') is-invalid @enderror"  value="{{ $cateitem->categoryValue}}"  @if (is_array($category = old('category')))@foreach ($category as $one){{ $one ==  $cateitem->categoryValue ? 'checked' : ' '; }}@endforeach @endif>
                                    ...
                                    <div id="collapseThreee" class="accordion-collapse collapse"
                                        ...
                                        <div class="accordion-body ms-4">
                                            <div class="row">
                                                ...
                                                <div class="col-md-4">
                                                    <div class="mb-3 form-check">
                                                        <input name="size[]" type="checkbox" class="form-check-input @error('size') is-invalid @enderror" value="{{ $sizeitem->size }}" @if (is_array($size = old('size')))@foreach ($size as $one){{ $one ==  $sizeitem->size  ? 'checked' : ' '; }}@endforeach @endif>
                                                        <label class="form-check-label productAddLable" for="{{ $sizeitem->size }}">{{ $sizeitem->size }}</label>
                                                    </div>
                                                </div>
                                                @endforeach
                                            </div>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="description" class="form-control rounded-0 addIteminput productTextArea @error('description') is-invalid @enderror" placeholder="Enter Item Description">{{ old('description') }}</textarea>
                    ...
                    <div class="col-6 d-grid">
                        <button type="submit" class="btn btn-dark btn-lg rounded-0 itemPublishBtn">PUBLISH</button>
                    </div>
                </form>
